...
class Plane(pygame.sprite.Sprite):
    ...
    def __init__(self, position, velocity, max_acceleration_vertical=0.01414, max_acceleration_parallel=0):
        ...
        self.weapons = {'missile': 6, 'homing_missile': 2}
...
class Control():
            ...
            if self.fire_operation in plane.weapons.keys():  # ['missile','homing_missile']
                if plane.weapons[self.fire_operation] > 0:
                    ...
                    if self.fire_operation == 'missile':
                        ...
                        pass
                        #  To be continue...这里还需要修改一下跟踪导弹的target_position的值,
                        # 或者是跟踪谁(对象,对象应该有position这个参数)

        self.mouse_target_pos = None  # 传值完，就对右键生成的坐标清理
        self.fire_operation = None  # 传值完，就对武器操作归零

        # pygame.sprite.spritecollide()
        # To be contine ... 碰撞还没写,其他差不多了.考虑发射出去别炸到自己,因为刚发射出去时是重叠的
    ...
    def draw(self):
        ...
        self.infomation.add(u"Mouse pos: %s " % self.mouse_select_pos)  # 显示上一次的鼠标位置
        self.infomation.add(u"Mouse pos: %s  -  %s   " % (
        self.player1[0].target_position, self.player1[1].target_position))  # 显示上一次的鼠标位置
    ...
    def main_loop(self):
        ...
        while not self.done:
            ...
            if self.senario_dir[self.senario_index] == 'Run':
                ...
                if pygame.time.get_ticks() - previous_time > 3000:  # 如果在run的运行状态下超过3秒,则切换为玩家模式
                    previous_time = None
                    self.senario_index = (self.senario_index + 1) % len(self.senario_dir.keys())
                # print 'run'
...
class Infomation(object):
    ...
    def show(self, screen):
        for i, text in enumerate(self.message_list):
            self.show_text(screen, (10, i * 10), text, (0, 0, 0))
        self.message_list = []

    def show_text(self, surface_handle, pos, text, color, font_size=14):
        """文字处理函数        """
        # 获取系统字体，并设置文字大小
